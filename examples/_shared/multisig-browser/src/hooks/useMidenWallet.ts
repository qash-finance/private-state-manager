...
import type { ExternalWalletState } from '../types';
...
export function useMidenWallet(adapter: MessageSignerWalletAdapter | null) {
  ...
  useEffect(() => {
    ...
    const handleConnect = (_address: string) => {
      ...
      const { scheme, publicKeyHex, commitment } = PublicKeyFormat.parse(publicKey);
      if (!commitment) {
        setConnectError(`Failed to derive commitment from ${scheme} public key (len=${publicKey.length})`);
        return;
      }
  ...
  const connect = useCallback(async () => {
    if (!adapter || connectingRef.current) {
      return;
    }
  ...
  const signBytes = useCallback(
    async (data: Uint8Array, kind: 'word' | 'signingInputs'): Promise<Uint8Array> => {
      if (!adapter) {
        throw new Error('Miden Wallet not connected');
      }
